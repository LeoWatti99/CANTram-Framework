...
#define OUTPUT_MASK 0xFF //Bitmask for flexible IOs (1=Output,0=Input)
...
void testdigitalModule_initialize(){
    ...
    OutputDefinition* csOutput = CANTramCore::getOutputDefinition(DIGITALMODULE_GPIO_START);
    ...
    OutputDefinition* csInput = CANTramCore::getOutputDefinition(DIGITALMODULE_GPIO_START+1);
 ...
 */
void test_digitalModule_setOutputs(){
    bool attachResult = CANTramCore::attachModule(&digitalModule);
    TEST_ASSERT_TRUE(attachResult);
    bool initResult = CANTramCore::initialize();
    TEST_ASSERT_TRUE(initResult);
    ...
    DigitalInput* inputInterface = digitalModule.getInterfaces()[0]->as<DigitalInput*>();
    INFO_PRINTLN("Using input interface: " + inputInterface->getName() + " to check outputs");
    DigitalOutput* outputInterface;

    for(int i=0;i<8;i++){
        //Check if pin is output
        if(!(OUTPUT_MASK & (1 << i))){
            INFO_PRINTLN("Skipping index "+String(i)+" as it is not an output");
            continue;
        }

        outputInterface = digitalModule.getInterfaces()[4 + i]->as<DigitalOutput*>();
        
        
        //activate output
        INFO_PRINTLN("Setting output interface "+outputInterface->getName()+" HIGH");
        outputInterface->setQ(1); //Set output HIGH
        CANTramCore::loop(); //Run a loop to apply the outputs
        vTaskDelay(100 / portTICK_PERIOD_MS); //Wait a bit

        INSTRUCTION_PRINTLN("Connect input "+inputInterface->getName()+" to output "+outputInterface->getName());
        while(!inputInterface->getQ()){
            CANTramCore::loop(); //Run a loop to update inputs
            vTaskDelay(2000 / portTICK_PERIOD_MS); //Wait a bit
        }

        //deactivate output 
        INFO_PRINTLN("Setting output interface "+outputInterface->getName()+" LOW");
        outputInterface->setQ(0); //Set output HIGH
        CANTramCore::loop(); //Run a loop to apply the outputs
        vTaskDelay(100 / portTICK_PERIOD_MS); //Wait a bit
        ...
        INFO_PRINTLN("OUTPUT "+outputInterface->getName()+" tested successfully.");
...
void setup(){
    ...
    RUN_TEST(test_digitalModule_setOutputs);
    UNITY_END();
}
